        ...
        <div class="row">
          <div class="col-xs-12">
            <div class="box">
              ...
              <!-- /.box-header -->
              <form role="form" action="<?=base_url();?>admin/save_set_access" method="post">              
                <div class="box-body">
                  <div class="form-group">
                    <input type="hidden" name="id_karyawan" value="<?php if(isset($id_karyawan)){echo $id_karyawan;}?>">
                    <input type="hidden" name="id_department" value="<?php if(isset($id_department)){echo $id_department;}?>">
                  </div>
                  <div class="form-group">
                    <h4>
                    <label><u>Room Access</u></label>
                    <br>
                    <?php
                    if (isset($listroom)) {
                      foreach ($listroom as $key => $value) {
                        ...
                        if ($value->deleted == 0){
                          if (isset($accessroom)) {
                            foreach ($accessroom as $key2 => $value2) {
                              if ($value2->id_room == $value->id_room) {
                                $xx++;
                              }
                            }
                          }
                    ?>
                        <input type="checkbox" name="<?=$value->id_room;?>" <?php if($xx > 0) echo "checked";?>> <?=$value->nama_room;?>
                        <br>
                    <?php
                        }
                      }
